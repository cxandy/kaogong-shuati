/**
 * region-crawler.mjs — 江浙沪·珠三角 江苏/上海/广东/深圳 近五年行测+申论真题爬取
 *
 ...
 *
 * 用法：
 *   node region-crawler.mjs                   完整爬取
 *   node region-crawler.mjs --only=申论       只爬申论
 *   node region-crawler.mjs --only=行测 --limit=1      只爬第一套做验证
 *   node region-crawler.mjs --region=上海      只爬指定地区（江苏/上海/广东/深圳）
...
const regionFilter = args.find((a) => a.startsWith('--region='))?.split('=')[1] || '全部';

const MIN_GAP = 4000;
const COOLDOWN = 90_000;
const FAIL_CAP = 6;

// [fenbiPaperId, label, region]
const PAPERS = {
  行测: [
    [4189914, '2025 江苏行测 A', '江苏'], [4189915, '2025 江苏行测 B', '江苏'], [4189916, '2025 江苏行测 C', '江苏'],
    [4162243, '2024 江苏行测 A', '江苏'], [7032843, '2024 江苏行测 B', '江苏'], [4162244, '2024 江苏行测 C', '江苏'],
    [214714,  '2023 江苏行测 A', '江苏'], [211891,  '2023 江苏行测 B', '江苏'],
    [162438,  '2022 江苏行测 A', '江苏'], [162375,  '2022 江苏行测 B', '江苏'], [162376,  '2022 江苏行测 C', '江苏'],
    [8472470, '2026 上海行测', '上海'], [4189943, '2025 上海行测 A', '上海'], [4189944, '2025 上海行测 B', '上海'],
    [198153,  '2023 上海行测 A', '上海'], [162380,  '2022 上海行测 B', '上海'],
    [8399765, '2026 广东行测', '广东'], [4435069, '2025 广东行测', '广东'], [7274732, '2024 广东行测', '广东'],
    [202665,  '2023 广东行测 县级', '广东'], [202741,  '2023 广东行测 乡镇', '广东'],
    [163233,  '2022 广东行测 县级', '广东'], [163234,  '2022 广东行测 乡镇', '广东'],
    [5232321, '2025 深圳行测', '深圳'], [7876607, '2024 深圳行测', '深圳'], [203154,  '2023 深圳行测', '深圳'], [162378,  '2022 深圳行测', '深圳'],
  ],
  申论: [
    [8155745, '2025 江苏申论 A、乡镇选调', '江苏'], [4641998, '2025 江苏申论 B', '江苏'], [7570793, '2025 江苏申论 C', '江苏'],
    [7479880, '2024 江苏申论 A、乡镇选调', '江苏'], [4508688, '2024 江苏申论 B', '江苏'], [4508690, '2024 江苏申论 C', '江苏'],
    [211730,  '2023 江苏申论 A、乡镇选调', '江苏'], [210668,  '2023 江苏申论 B', '江苏'], [222029,  '2023 江苏申论 C', '江苏'],
    [162384,  '2022 江苏申论 A、普通选调', '江苏'], [162386,  '2022 江苏申论 B', '江苏'], [162385,  '2022 江苏申论 C', '江苏'],
    [5319839, '2026 上海申论', '上海'], [6623483, '2025 上海申论 A', '上海'], [8664957, '2025 上海申论 B', '上海'],
    [8373050, '2024 上海申论 A', '上海'], [5934292, '2024 上海申论 B', '上海'], [209201,  '2023 上海申论 A', '上海'], [196011,  '2022 上海申论 B、定向选调', '上海'],
    [8485529, '2026 广东申论 省市', '广东'], [8485530, '2026 广东申论 县镇', '广东'], [6308806, '2026 广东申论 行政执法', '广东'],
    [7613274, '2025 广东申论 省市', '广东'], [4549046, '2025 广东申论 县镇', '广东'], [6196434, '2025 广东申论 行政执法', '广东'],
    [7528577, '2024 广东申论 省市', '广东'], [7528573, '2024 广东申论 县镇', '广东'], [4508693, '2024 广东申论 行政执法', '广东'],
    [203127,  '2023 广东申论 县级', '广东'], [212404,  '2023 广东申论 乡镇', '广东'],
    [182065,  '2022 广东申论 县级', '广东'], [180035,  '2022 广东申论 乡镇', '广东'],
    [4248031, '2025 深圳申论 一卷', '深圳'], [5719804, '2025 深圳申论 二卷', '深圳'],
    [5934276, '2024 深圳申论 二卷', '深圳'],
    [211879,  '2023 深圳申论 一卷', '深圳'], [211878,  '2023 深圳申论 二卷', '深圳'],
    [162381,  '2022 深圳申论 一卷', '深圳'], [162382,  '2022 深圳申论 二卷', '深圳'],
  ],
};
...
function insertPaper(subj, sheet, chapters, region) {
  ...
  p.run(
    sheet.paperId, subj.subject, subj.subjectName, region, sheet.name, sheet.questionCount,
...
// ---------- 行测爬取 ----------
async function crawlXingce(paperId, label, region) {
  console.log(`\n[行测][${region}] ${label} (${paperId})`);
  ...
  insertPaper(SUBJECTS.行测, sheet, sheet.chapters, region);
...
// ---------- 申论爬取 ----------
async function crawlShenlun(paperId, label, region) {
  console.log(`\n[申论][${region}] ${label} (${paperId})`);
  ...
  writeQuestions(paperId, rows);
  insertPaper(SUBJECTS.申论, sheet, sheet.chapters, region);
...
async function main() {
  ...
  console.log(`开始爬取江浙沪·珠三角真题 ${extra}  cookie: ${cookie.slice(0, 14)}…`);
  const order = ['行测', '申论'];
  let done = 0, skip = 0, fail = 0;
  for (const kind of order) {
    if (only !== '全部' && only !== kind) continue;
    let papers = PAPERS[kind];
    if (regionFilter !== '全部') papers = papers.filter((p) => p[2] === regionFilter);
    if (limit) papers = papers.slice(0, limit);
    for (const [pid, label, region] of papers) {
      if (paperDone(pid)) { console.log(`跳过已完成 ${kind} ${label}`); skip++; continue; }
      try {
        if (kind === '行测') await crawlXingce(pid, label, region);
        else await crawlShenlun(pid, label, region);
        done++;
      } catch (e) {
        fail++;
